...
export function useTokenBalance(symbol, address = '') {
  ...
  const updateBalance = useCallback(() => {
    ...
    tokenContract.balanceOf(requestedAddress).then(balance => {
      console.log(`${requestedAddress} ${balance}`)
      if (!cancelled) {
        console.log(`The recipient ${requestedAddress} owns ${tokenContract.address} ${balance.toString()}`)
        setBalance(balance)
      }
    })
  ...
  return useCallback(
    async hash => {
      ...
      try {
        ...
        const amount = parsedTransferLog.args.amount
